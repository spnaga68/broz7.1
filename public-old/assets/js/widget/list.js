...
var Widgetlist = {};
var SDWIDGETLIST = function(widget){
        var datepicker = false;
        A = widget; 
        A.totalList = 0;
        A.app = {},A.loadData = A.loadData ? A.loadData : true, A.curpage = A.curpage ? A.curpage : 1, A._urlparams = {};
        A.init = function(app)
        {
            A.app = $.extend (app,{
                toParams :function(searchUrl) {
                    ...
                    $.each(params, function(index, param){
                        ...
                        var key = keyPair[0];
                        var value = decodeURIComponent(keyPair[1]);
            ...
            try {
                this.app.urlparams = jQuery.parseJSON(this.app.urlparams);
            } catch(e) {
            ...
            if (this.app.useScroll) { 
                $(window).scroll(function() { 
                    if(A._isScrolledIntoView("#"+ A.app.container+" .loadmoreresults")) {
        ...
        A.massAction = function(options){
            ...
            var $this = this;
            var inputelement;
            var _op = $.extend({items:{},formid:_cur.container+'_massaction-form',submitId:_cur.container+'_massaction-submit',confirmtext:'',elementname:'',lists:{}},options);
            var init =  {
                prepare:function()
                {
                ...
                checkbox:function(ele)
                {
                    ...
                    A.totalList = $('#'+ _cur.container+' tr.item_columns input[type="checkbox"]:checked').length;
                    if(A.totalList >= 0 && A.totalList < $('#'+ _cur.container+' tr.item_columns input[type="checkbox"]').length) {
        ...
        A.addVarToScrollUrl = function(varName, varValue)
        {
            this.app.scrollurl = A._addVarToUrl( this.app.scrollurl,varName, varValue);
            return  this.app.scrollurl;
        };
        ...
        A.Paging = function(event)
        {
            ...
            if(element.attr('href') ){
                A.app.url = element.attr('href');
                A.reloadUrl( A.app.url);
            }
            event.preventDefault();
            return false;
        };
        A._isScrolledIntoView = function(elem)
		{ 
			var $elem = $(elem); 
			var $window = $(window);
        ...
        A.loadMore = function()
        { 
            var params = this.app.urlparams; 
            var perpage = params.limit ? params.limit : 50; 
            var _op = this.app; 
            this.loadData = false; 
            
            if (this.curpage * perpage == _op.totalItem) {
                if (_op.totalItem > perpage) {
                    this.curpage++;
            ...
            if (this.curpage * perpage > _op.totalItem) {
                if (_op.totalItem > perpage) {
                    $(".loadmoreresults").html(SD.translate.get('Thats it!!!'));
                } 
                return false;
            }
            if (this.curpage * perpage < _op.totalItem) {
                ...
                this.curpage++;
                A.addVarToScrollUrl('page',this.curpage);
                A.scroll(A.app.scrollurl); 
            }
        };
        
        A.scroll = function(url)
        { 
            A.totalList = 0;
            var _op = this.app;  
            $.ajax({
                ...
                success: function (data) {
                     ...
                     if(IS_JSON) {
                        ...
                        if (_op.useScroll) {
                            $('#'+ _op.container+' table tbody tr:last').after(response);
                        } else {
                            var divId = $('#'+ _op.container);
                            divId.html(response);
                        } 
                     }
                    if(_op.callback !='undefined') {
                        //console.log(_op.callback);
                        //_op.callback();
                    }
                    A.loadData = true;
        ...
        A.RowEdit = function(event)
        {
            ...
            if(element.attr('data-title') && element.attr('data-title')!="#"){
                window.location = element.attr('data-title');
            }
            event.preventDefault();
            return;
        }

        A.FilterList = function(container)
        {
            var elements = []; 
            container || (container = this.app.container); 
            $('#'+ container+' .filter input').each(function(k,v){
                if(v.value && v.value.length) { 
                    $ele = v.getAttribute('name')+'='+v.value;
                    elements.push($ele);
                }
            });
            $('#'+ container+' .filter select').each(function(k,v){
                if(v.value && v.value.length) {
                    $ele = v.getAttribute('name')+'='+v.value;
                    elements.push($ele);
                }
        ...
        A.Sort = function(event){
            ...
            if(element.attr('name') && element.attr('title')){
                A.addVarToUrl( A.app.sortvar, element.attr('name'));
                A.addVarToUrl( A.app.dirvar, element.attr('title'));
                A.reloadUrl( A.app.url);
            }
            event.preventDefault();
            return false;
