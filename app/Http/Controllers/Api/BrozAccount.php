...
class BrozAccount extends Controller
{
   ...
   /* public function signup_fb_user(Request $data)
    ...
    public function delete_address(Request $data)
    {
        ...
        if(count($address)>0)
        {
            ...
            ->where('user_id', $post_data['user_id'])
            ->delete();
            $result = array("response" => array("httpCode" => 200 , "Message" => trans("messages.Address deleted Successfully"), "card_detail"=>$address));
        }
        else
        {
            $result = array("response" => array("httpCode" => 200 , "Message" => trans("messages.No Address found")));
        }
        return json_encode($result,JSON_UNESCAPED_UNICODE);
    }
    ...
    public function update_profile(Request $data)
    {
        ...
        $rules = [
            ...
            'phone'      => ['required', 'max:50','regex:/\(?([0-9]{3})\)?([ .-]?)([0-9]{3})\2([0-9]{4})/'],
            'gender'     => ['required'],
        ];
        ...
        else 
        {
            ...
            $users->save();
            $result = array("response" => array("httpCode" => 200, "status" => true, "Message" => trans("messages.User information has been updated successfully")));
        }
    ...
    { 
		...
		$post_data = $data->all();
		print_r($post_data);//exit('test3773');
        if(isset($post_data['language']) && $post_data['language'] == 2)
        {
            App::setLocale('ar');
        }
 ...
 $user->mobile= ($user->mobile != '')?$user->mobile:'';
    ...
    public function store_address(Request $data)
    {
      ...
      if(isset($post_data['language']) && $post_data['language']==2)
        ...
        else 
        {
            ...
            $address->longtitude    = $post_data['longtitude'];
            ...
            $address->save();
            $result = array("response" => array("httpCode" => 200, "status" => true, "Message" => trans("messages.Address Details Saved Successfully")));
        }
        return json_encode($result,JSON_UNESCAPED_UNICODE);
    }
    ...
    {    
        ...
        $address = DB::table('user_address')
                ->select('user_address.address','user_address.latitude','user_address.longtitude','user_address.address_type','user_address.user_id','user_address.id as address_id')
                ->where('user_id','=',$post_data['user_id'])
        ...
        if(count($address)>0)
        {
            $result = array("response" => array("httpCode" => 200 , "Message" => trans("messages.Address detail"), "address_detail"=>$address));
        }
        else
        {
            $result = array("response" => array("httpCode" => 400 , "Message" => trans("messages.No Address found"),"address_detail"=>$address));
        }
        return json_encode($result,JSON_UNESCAPED_UNICODE);
    }
    ...
    public function update_address(Request $data)
    {
        ...
        else 
        {
            $check_auth = JWTAuth::toUser($post_data['token']);
            $Address = new Address;
            $Address = Address::find($post_data['address_id']);
            $Address->name      = $post_data['name'];
            $Address->address      = $post_data['address'];
            $Address->city_id    = $post_data['city'];
            $Address->country_id    = $post_data['country'];
            $Address->modified_date = date("Y-m-d H:i:s");
            $Address->user_id    =  $post_data['user_id'];
            $Address->save();
            $result = array("response" => array("httpCode" => 200, "status" => true, "Message" => trans("messages.Address Details Saved Successfully")));
            ...
            try {
                ...
                if(count($vendors) > 0 )
                {
                    ...
                    foreach($vendors as $ven)
                    {
                        ...
                        $vendor_list[$v]['outlets_delivery_charges_fixed'] = $ven->outlets_delivery_charges_fixed;$vendor_list[$v]['minimum_order_amount'] = $ven->minimum_order_amount;
