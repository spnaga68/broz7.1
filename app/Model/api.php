...
class api extends Model
{
	...
	public function getOutlets($outlet_type,$vendor_id) 
    {	
    	$user_array = array("outlet_category" => $outlet_type,"vendor_id" => $vendor_id);
    	//print_r($user_array);exit();
		$method = "POST";
		$data = array('form_params' => $user_array);
		$response = $this->call_api($data, '/api/outletDetails', $method);
		//echo"<pre>";print_r($response);exit();
		return $response;
		
	}
	public function paymentHsitory($cust_id)
	{
    	$user_array = array("customer_id" => $cust_id);
    	//print_r($user_array);exit();
		$method = "POST";
		$data = array('form_params' => $user_array);
		$response = $this->call_api($data, '/api/PaymentHistory', $method);
		//echo"<pre>";print_r($response);exit();
		return $response;    	
