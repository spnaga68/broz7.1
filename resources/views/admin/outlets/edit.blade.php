...
<div class="grid simple">
	...
	{!!Form::open(array('url' => ['update_outlet',$data->id], 'method' => 'post','class'=>'panel-wizard','id'=>'outlet_edit_form','files' => true));!!}
        ...
        <div class="tab-pane active" id="login_info">
					...
					<div class="col-sm-7">
						...
						<?php $i = 0; foreach($languages as $langid => $language):?>
							<div class="input-group translatable_field language-<?php echo $language->id;?>" <?php if($i > 0):?>style="display: none;"<?php endif;?>>
								<input type="text" name="outlet_name[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>" placeholder="<?php echo trans('messages.Outlet Name').' '.trans('messages.'.'('.$language->name.')');?>" class="form-control" value="{{$infomodel->getLabel('outlet_name',$language->id,$data->id)}}" maxlength="32" />
					...
					<div class="col-sm-7">
						<select name="vendor" id="vendor_id" class="form-control" >
							...
							@foreach($vendors as $list)
								<option value="{{$list->id}}" <?php echo ($data->vendor_id==$list->id)?'selected="selected"':'';  ?> >{{$list->vendor_name}}</option>
							@endforeach
						</select>
					</div>
			   </div>
			...
			<div class="tab-pane" id="delivery_info">
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						<input type="text" name="minimum_order_amount" value="{!! $data->minimum_order_amount !!}"  maxlength="255" placeholder="@lang('messages.Minimum Order Amount')"  class="form-control"  />
