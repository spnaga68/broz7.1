...
<ul class="nav nav-tabs"></ul>
	...
	<div class="tab-pane active" id="home3">
				...
				<div class="form-group">
				 ...
				 <div class="form-group">
				<label class="col-sm-2 control-label">@lang('messages.Language')<span class="asterisk">*</span></label>
				<div class="col-sm-10">
				  <select id="language_type" name="language_type" class="form-control" >
					<option value="">@lang('messages.Select Language')</option>
						@if (count(getLanguageList()) > 0)
							@foreach (getLanguageList() as $language)
								<option value="{{ $language->id }}" <?php echo ($data->language_type == $language->id)?'selected="selected"':''; ?> >{{ ucfirst($language->name) }}</option>
							@endforeach
						@endif
				  </select>
				</div>
