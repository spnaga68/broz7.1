...
<div class="contentpanel">
	...
	<table id="vendor-table" class="table table-striped table-bordered responsive">
		<thead>
			<tr class="headings">
			<th><input type="checkbox"  id="bulkDeletevendor"/> <button id="deleteTrigervendor">@lang('messages.Delete')</button></th> 
...
<script>
$(function() {
	var checkbox=$('#deleteTrigervendor').show();
    $('#vendor-table').DataTable({
	 ...
	 $(".deleteRow").on('click',function() { 
			...
			if(status1){
			$('#deleteTrigervendor').show();
			}else {
				$('#deleteTrigervendor').hide();
			}
...
$("#bulkDeletevendor").on('click',function() { // bulk checked
    ...
    $('#deleteTrigervendor').on("click", function(event){
        ...
        if( $('.deleteRow:checked').length > 0 ){
			if (confirm("Are you sure want to delete?")) {
				...
				url = '{{url('admin/vendors/bulkdelete')}}';
				...
				success: function(result) {
					...
					$('#deleteTrigervendor').hide();
					$('#bulkDeletevendor').prop("checked",false);
					//oTable.ajax.reload();
