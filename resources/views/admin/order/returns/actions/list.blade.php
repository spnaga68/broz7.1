...
<div class="media">
	...
	<div class="media-body">
		...
		<h4>@lang('messages.Return Actions List')</h4>
	</div>
</div><!-- media -->
</div><!-- pageheader -->
...
<a class="btn btn-primary tip" href="{{ URL::to('admin/returnaction/create') }}" title="Add New">@lang('messages.Add New')</a>
</div>
</div>
 ...
 <table id="currencyTable" class="table table-striped table-bordered responsive">
    <thead>
        <tr class="headings">
            ...
            <th>@lang('messages.Return Action Name')</th>  
            <th>@lang('messages.Actions')</th> 
        </tr>
    </thead>
         @if (count($returnactions) > 0 )
    <tbody>
       <?php $i=1; ?>
    @foreach($returnactions as $key => $value)
        <tr>
            <td>{{$i}}</td>
            <td>{{ $value->name }}</td>
...
$(function() {
    $('#currencyTable').DataTable({
        ...
        ajax: '{!! route('ajaxreturnaction.data') !!}',
        "order": [],
		"columnDefs": [ {
		  "targets"  : 'no-sort',
		  "orderable": false,
