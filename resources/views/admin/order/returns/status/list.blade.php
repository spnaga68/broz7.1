...
<div class="media">
	...
	<div class="media-body">
		...
		<h4>@lang('messages.Return Status List')</h4>
	</div>
</div><!-- media -->
</div><!-- pageheader -->
...
<a class="btn btn-primary tip" href="{{ URL::to('admin/returnstatus/create') }}" title="Add New">@lang('messages.Add New')</a>
</div>
</div>
 ...
 <table id="currencyTable" class="table table-striped table-bordered responsive">
    <thead>
        <tr class="headings">
            ...
            <th>@lang('messages.Return Status Name')</th>  
    ...
    <tbody>
       <?php $i=1; ?>
    @foreach($returnstatuses as $key => $value)
        <tr>
            <td>{{$i}}</td>
            <td>{{ $value->name }}</td>
...
$(function() {
    $('#currencyTable').DataTable({
        ...
        ajax: '{!! route('ajaxreturnstatus.data') !!}',
        "order": [],
		"columnDefs": [ {
		  "targets"  : 'no-sort',
		  "orderable": false,
