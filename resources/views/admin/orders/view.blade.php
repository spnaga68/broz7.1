...
<div class="row panel panel-default">
	<div class="grid simple">
		...
		<ul class="nav nav-justified nav-wizard nav-pills">
			...
			<li @if(old('tab_info')=='delivery_info') class="active" @endif ><a href="#delivery_info" class="delivery_info" data-toggle="tab"><strong><?php if ($delivery_details[0]->order_type == 1) {
	echo trans('messages.Delivery Information');
} else {
	echo trans('messages.Pickup Information');
}
?></strong></a></li>
			...
			<div class="tab-pane active" id="login_info">
					...
					<div class="form-group">
						...
						<?php if ($currency_side == 1) {?>
						       <label class="col-sm-4 control-label">{!!$currency_symbol!!}{!! $delivery_details[0]->total_amount !!} </label>
						  <?php } else {?>
						       <label class="col-sm-4 control-label">{!! $delivery_details[0]->total_amount !!} {!!$currency_symbol!!}</label>
						  <?php }?>
						</div>
					</div>
					<div class="form-group">
			...
			<div class="tab-pane" id="vendor_info">
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
					  ...
					  <label class="col-sm-4 control-label">{!! $currency_symbol !!}</label>

					</div>
				</div>
			</div>
			<div class="tab-pane" id="delivery_info">
				<legend> <?php if ($delivery_details[0]->order_type == 1) {
	echo trans('messages.Delivery Information');
} else {
	echo trans('messages.Pickup Information');
}
?></legend>
				...
				<div class="form-group">
					...
					</div><?php $order_history1[0]->order_attachment = '';?>
					<div class="form-group">
...
foreach ($order_items as $items) {
	?>
							<tr>
								...
								<?php if ($currency_side == 1) {?>
								     <td class="text-right">{{$currency_symbol.$items->item_cost}}</td>
									 <td class="text-right"><?php echo $currency_symbol . $items->item_cost * $items->item_unit; ?></td>
								<?php } else {?>
								     <td class="text-right">{{$items->item_cost.$currency_symbol}}</td>
									 <td class="text-right"><?php echo $items->item_cost * $items->item_unit . $currency_symbol; ?></td>
								<?php }?>
...
$subtotal += $items->item_cost * $items->item_unit;
	?>
							<tr>
								...
								<?php } else {?>
								    <td class="text-right">{{$subtotal.$currency_symbol}}</td>
								<?php }?>
							</tr>
							...
							<tr>
								...
								<?php if ($currency_side == 1) {?>
								    <td class="text-right">{{$currency_symbol.$items->delivery_charge}}</td>
								<?php } else {?>
								    <td class="text-right">{{$items->delivery_charge.$currency_symbol}}</td>
								<?php }?>
							</tr>
							<?php }?>
							<tr>
								...
								<?php if ($currency_side == 1) {?>
								    <td class="text-right">{{$currency_symbol.$items->service_tax}}</td>
								<?php } else {?>
								    <td class="text-right">{{$items->service_tax.$currency_symbol}}</td>
								<?php }?>
							</tr>
							...
							<tr>
								...
								<?php if ($currency_side == 1) {?>
								   <td class="text-right">- {{$currency_symbol.$items->coupon_amount}}</td>
								<?php } else {?>
								   <td class="text-right">- {{$items->coupon_amount.$currency_symbol}}</td>
								<?php }?>
							</tr>
							<?php }?>
							<tr>
								...
								<?php if ($currency_side == 1) {?>
								<td class="text-right">{{$currency_symbol.$items->total_amount}}</td>
								<?php } else {?>
								<td class="text-right">{{$items->total_amount.$currency_symbol}}</td>
								<?php }?>
							</tr>
								<?php }?>
					</tbody>
...
if ($delivery_details[0]->order_type == 2) {
	...
	?>
									 <option value="{{$order_status_list['id']}}">{{$order_status_list['name']}}</option> <?php } else {
