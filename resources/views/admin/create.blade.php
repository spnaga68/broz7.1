	...
	{!!Form::open(array('url' => 'product_create', 'method' => 'post','class'=>'panel-wizard','id'=>'product_form','files' => true));!!}
		<div class="tab-content tab-content-simple mb30 no-padding" >
			...
			<div class="form-group">
				...
				<div class="form-group">
                            <label class="col-sm-3 control-label">@lang('messages.Vendor type') <span class="asterisk">*</span></label>
                            <div class="col-sm-7">
							 <select name="vendor_type" id="vendor_type" name="vendor_type"  class="form-control">
                                    <option value="">@lang("messages.Select vendor type")</option>
                                    <option value="1" <?php if (old('vendor_type') == 1) {echo "selected";}?>>@lang("messages.All")</option>
                                    <option value="2" <?php if (old('vendor_type') == 2) {echo "selected";}?>>@lang("messages.Select Vendors")</option>
...
if (!empty(old('head_category'))) {
	$vendor = getCategoryVendorLists(old('head_category'));?>
...
$vendors = getCategoryVendorLists(old('head_category')); ?>

@foreach($vendors as $list)
<option value="{{$list->id}}" <?php echo (old('vendor')==$list->id)?'selected="selected"':''; ?> >{{$list->vendor_name}}</option>
@endforeach
<?php } ?>
</select>
</div>
...
<?php
if(!empty(old('vendor'))){
$outlet = getOutletList(old('vendor'));
?>
...
<button type="reset" title="Cancel"  class="btn btn-default" onclick="window.location='{{ url('/dynamic') }}'">@lang('messages.description')</button>
