					...
					<tr>
						...
						<td>
							...
							@elseif($value->approve_status==1)
								<span class="label label-success" id="<?php echo 'approve_status_'.$value->id;?>">@lang('messages.Completed')</span>
							@elseif($value->approve_status==2)
								<span class="label label-danger" id="<?php echo 'approve_status_'.$value->id;?>">@lang("messages.Cancelled")</span>
							@endif
						</td>
					<?php if(hasTask('orders/approve_fund_status')) { ?>
							<td>
								<div class="<?php echo 'request_amount_'.$value->id;?>">
									...
									<?php if($value->approve_status==0) { ?>
										<option value="">Selectg Option</option>
								    ...
								    <?php } else if($value->approve_status==1) { ?>	
								    <option <?php echo ($value->approve_status==1)?"selected='selected'":"";?> value="1">@lang('messages.Completed')</option>
			...
			success: function (resp) {
				...
				if(resp.data==1)
				{
					...
					if(status==1)
					{
						...
						$('#approve_status_'+cid).text('Completed').attr('class','label label-success'); 
					}
					else if(status==2){
						$('#approve_status_'+cid).text('Cancelled').attr('class','label label-danger'); 
					}
				}
			},
