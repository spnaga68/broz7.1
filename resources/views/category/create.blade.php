...
<div class="contentpanel">
	...
	<div class="tab-content mb30">
		<div class="tab-pane active" id="home3">
			...
			<div class="form-group" id="head_category" style="display:none;">
				<label class="col-sm-2 control-label">@lang('messages.Vendor Category')<span class="asterisk">*</span></label>
				<div class="col-sm-10">
					<?php  $old =old('head_category'); $cate=array(); if($old){  $cate=$old; } ?>
					 <select name="head_category[]" id="head_categories" multiple data-placeholder="@lang('messages.Select Category')"  class="width300"  >
		...
		<div class="form-group" id="sub_category_data" style="<?php echo (old('is_category_type')==1)?'display:block;':'display:none;'; ?>">
					...
					<div class="col-sm-10">
						<select id="main_category" name="main_category" class="form-control" >
							...
							<?php
								...
								@if (count($cdata) > 0 ) && old('main_category'))
									@foreach ($cdata as $key => $val)
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
				  <input type="number" name="sort_order" value="{!! old('sort_order') !!}" min="1" max="100" placeholder="@lang('messages.Sort order')"  class="form-control"  />
				</div>
			</div>
			<div class="form-group">
					...
					<div class="row">
						<div class="col-sm-10" style="padding-right: 0;">
						   <textarea class="form-control" rows="3" cols="150"  name="description[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>"  placeholder="<?php echo trans('messages.Description').trans('messages.'.'('.$language->name.')');?>" class="form-control" >{!! Input::old('description.'.$language->id) !!}</textarea>
						   </div>
			...
			<div class="form-group">
					...
					<div class="col-sm-10">
						<?php $i = 0; foreach($languages as $langid => $language):?>
						<div class="input-group translatable_field language-<?php echo $language->id;?>" <?php if($i > 0):?>style="display: none;"<?php endif;?>>
							  <input type="text"  name="meta_title[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>"  placeholder="<?php echo trans('messages.Meta Title').trans('messages.'.'('.$language->name.')');?>" class="form-control" value="{!! Input::old('meta_title.'.$language->id) !!}"  />
			...
			<div class="form-group">
                ...
                <div class="col-sm-10">
                    <?php $i = 0; foreach($languages as $langid => $language):?>
                    <div class="input-group translatable_field language-<?php echo $language->id;?>" <?php if($i > 0):?>style="display: none;"<?php endif;?>>
                          <input type="text" name="meta_keywords[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>"  placeholder="<?php echo trans('messages.Meta Keywords').trans('messages.'.'('.$language->name.')');?>" class="form-control" value="{!! Input::old('meta_keywords.'.$language->id) !!}"  />
        ...
        */ ?>
	    ...
	    <div class="form-group">
			<label class="col-sm-2 control-label">@lang('messages.White image')<span class="asterisk">*</span></label>
			<div class="col-sm-10">
				<input type="file" name="category_white_image"/>
			</div>
		</div>	
		<div class="form-group">
