...
<div class="contentpanel">
	...
	<ul class="nav nav-justified nav-wizard nav-pills">
		...
		{!!Form::open(array('url' => ['managers/update_product',$data->id], 'method' => 'post','class'=>'panel-wizard','id'=>'edit_product_form','files' => true));!!}
			...
			<div class="tab-pane active" id="login_info">
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						...
						<?php $cdata = getVendorsubCategoryLists(Session::get('manager_vendor'));  ?>
						@if (count($cdata) > 0)
...
$('#edit_product_form').submit(function(evt) {
        ...
        var c_url = '/managers/update_product/{{$data->id}}';
        token = $('input[name=_token]').val();
        $.ajax({
            url: c_url,
            headers: {'X-CSRF-TOKEN': token},
            ...
            success: function (resp)
            {
                ...
                if(data.status == 200)
                { 
                    window.location='{{ url('managers/products') }}';
                }
                else
                { 
                    $('.alert-danger').show();
