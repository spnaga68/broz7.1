...
<div class="contentpanel">
	...
	@endif
	{!!Form::open(array('url' => 'managers/return_orders', 'method' => 'get','class'=>'tab-form attribute_form','id'=>'sales_order_search_form','files' => true));!!}
		...
		<div class="form-group">
			...
			<button type="reset" title="@lang('messages.Reset')" class="btn btn-default" onclick="window.location='{{ url('managers/return_orders') }}'">@lang('messages.Reset')</button>
			<?php if(Input::get('search')){ ?>
				<button type="reset" title="@lang('messages.Export')" class="btn btn-default" onclick="window.location='{{ url('managers/return_orders?export=1&from='.Input::get('from').'&to='.Input::get('to').'&outlet='.Input::get('outlet').'&search='.Input::get('search')) }}'">@lang('messages.Export')</button>
			<?php } ?>
		</div>
	{!!Form::close();!!}
		...
		<table id="return_orders" class="table table-striped table-bordered responsive">
			<thead>
				<tr class="headings">
					...
					<th>@lang('messages.Restaurant Name')</th>
				...
				<tbody>
					...
					@foreach($data as $key => $value)
						<tr> 
							...
							<td>
								<div class="btn-group">
									<a href="{{URL::to('managers/return_orders_view/'.$value->id)}}" class="btn btn-xs btn-white" title="@lang('messages.View')"><i class="fa fa-file-text-o"></i>@lang("messages.View")</a>
								</div>
							</td>
						</tr>
						<?php $i++; ?>
