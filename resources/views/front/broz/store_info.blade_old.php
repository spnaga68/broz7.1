                        ...
                        <div class="bakery_sections">
                            <div class="kakery_slider">
                                ...
                                <div class="owl-carousel">
                                    <?php foreach($store->most_sell_products as $most_sell_products=>$spro){ 
                                        if($sp <=10) {  ?>
                                            <div class="item">
                                                <?php if(($spro->original_price-$spro->discount_price > 0) && ($spro->original_price > 0)) { ?>
                                                    <div class="price_dic">
                                                        <h6>@lang('messages.Off')</h6>
                                                        <?php if($currency_side == 1) { ?>
                                                            <p><?php echo $currency_symbol; ?>{{ $spro->original_price-$spro->discount_price }} </p>
                                                        <?php } else { ?>
                                                            <p>{{ $spro->original_price-$spro->discount_price }} <?php echo $currency_symbol; ?></p>
                                                        <?php } ?>
                                                    </div>
                                                <?php } ?>
                                                <div class="item_common_new">
                                                    <div class="store_list_img">
                                                        <?php if(file_exists(base_path().'/public/assets/admin/base/images/products/list/'.$spro->product_image)) { ?>
                                                            <a href=" {{ URL::to('/product/info/'.$data->url_index.'/'.$spro->product_url)  }}" title=""> <img  title="{{ $spro->product_name }}" alt="{{ ucfirst(strtolower($spro->product_name)) }}"  src="<?php echo url('/assets/admin/base/images/products/list/'.$spro->product_image.''); ?>" ></a>
                                                        <?php } else{  ?>
                                                            <a href="{{ URL::to('/product/info/'.$data->url_index.'/'.$spro->product_url) }}" title=""><img src="{{ URL::asset('assets/admin/base/images/products/product.png') }}" alt="{{ ucfirst(strtolower($spro->product_name)) }}"></a>
                                                        <?php } ?>
                                                    </div>
                                                    <div class="store_list_desc">
                                                        <a href=" {{ URL::to('/product/info/'.$data->url_index.'/'.$spro->product_url)  }}"  title="{{ strtolower($spro->product_name) }}">{{ ucfirst(strtolower(trim($spro->product_name))) }}  ({{ $spro->weight }}{{ $spro->unit }})</a>
                                                        <?php if($currency_side == 1) { ?>
                                                            <h3>@if(($spro->original_price-$spro->discount_price > 0) && ($spro->original_price > 0))<span class="marked_price"><?php echo  $currency_symbol; ?>{{ $spro->original_price }} </span>@endif<span class="nrl_price"><?php echo $currency_symbol; ?>{{ $spro->discount_price }} </span></h3>
                                                        <?php } else { ?>
                                                            <h3>@if(($spro->original_price-$spro->discount_price > 0) && ($spro->original_price > 0))<span class="marked_price">{{ $spro->original_price }} <?php echo $currency_symbol; ?></span>@endif<span class="nrl_price">{{ $spro->discount_price }} <?php echo $currency_symbol; ?></span></h3>
                                                        <?php } ?>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="item_common_new">
                                                    <div class="store_list_img">
                                                        <?php if(file_exists(base_path().'/public/assets/admin/base/images/products/list/'.$pro->product_image)) { ?>
                                                            <a href=" {{ URL::to('product/info/'.$data->url_index.'/'.$pro->product_url)  }}" title=""> <img  title="{{ $pro->product_name }}" alt="{{ ucfirst(strtolower($pro->product_name)) }}"  src="<?php echo url('/assets/admin/base/images/products/list/'.$pro->product_image.''); ?>" ></a>
                                                        <?php } else{  ?>
                                                            <a href="{{ URL::to('product/info/'.$data->url_index.'/'.$pro->product_url) }}" title=""><img src="{{ URL::asset('assets/admin/base/images/products/product.png') }}" alt="{{ ucfirst(strtolower($pro->product_name)) }}"></a>
                                                        <?php } ?>
                                                    </div>
                                                    <div class="store_list_desc">
                                                        <a href=" {{ URL::to('product/info/'.$data->url_index.'/'.$pro->product_url)  }}"  title="{{ strtolower($pro->product_name) }}">{{ ucfirst(strtolower(trim($pro->product_name))) }}  ({{ $pro->weight }}{{ $pro->unit }})</a>
                                                        <?php if($currency_side == 1) { ?>
                    ...
                    <div class="store_info_sections">
                        ...
                        <div class="col-md-4">
                            <div class="sto_info_add">
                                ...
                                <p>{{ $data->outlets_contact_address }}.<br>
                                @lang('messages.Contact Number') {{ $data->outlet_phone_number }}</p>
                        ...
                        <div class="col-md-8">
                            <div class="map_sections">
                                ...
                                <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyB_0Bwt4UE1kkofS5hBFldXDKsH0Jgi1XE"></script>
