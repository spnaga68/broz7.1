                                ...
                                <div class="store_info_respon">
                                    ...
                                    <div class="responsive_store_info">
                                        ...
                                        <ul>
                                            ...
                                            <?php $i=1; foreach($vcategories as $cate){ //print_r($cate);die;?>
                                                <?php  if($i <= 6){ ?> 
                                                ...
                                                <?php } ?>
                                            <?php $i++; } ?>
                                            <?php if(count($vcategories)>=6){ ?> 
                                                <li>
                                                    <a href="javascript:;" title="@lang('messages.More')" id="open_drop_me1">@lang('messages.More')</a>
                                                    <div class="toogle_drop_menu">
                                                        <ul>
                                                            <?php $i=1;  foreach($vcategories  as $cate){ ?>
                                                                <?php if($i>6){ ?> 
                                                                    <li><a href="{{ URL::to('/products/'.$cate->url_key.'/'.$data->url_index.'/'.$cate->category_id) }}" class="category_clik" attr-id="{{ ucfirst(strtolower($cate->category_name)) }}" title="{{ ucfirst(strtolower($cate->category_name)) }}">{{ ucfirst(strtolower($cate->category_name)) }}</a></li>
                                                                <?php } ?>
                                                            <?php $i++; } ?>
                                                        </ul>
                                                    </div>
                                ...
                                <div class="noraml_store_info">
                                    <ul>
                                        <li><?php if(count($categories)>0){?>
                                            <a href="{{ URL::to('/products/all/'.$data->url_index) }}" title="@lang('messages.All')">@lang('messages.All') </a>
											
                                        </li>
                                        <?php } ?>
                                        <?php $i=1; foreach($categories as $cate){ ?>
                                            <?php  if($i <= 5){ ?> 
                                                <li><a href="{{ URL::to('/products/'.$cate['url_key'].'/'.$data->url_index.'/'.$cate['category_id']) }}" class="category_clik" attr-id="{{ ucfirst(strtolower($cate['category_name'])) }}" title="{{ ucfirst(strtolower($cate['category_name'])) }}">{{ ucfirst(strtolower($cate['category_name'])) }} </a><span class="megamenu_arrows"></span>
                                                <?php if(count($cate['subcategory']) > 0 ) { ?>
													<div class="level_one_drop col-md-12">
														<div class="col-md-8 padding_left0">
															<div class="common_megamenu_sec_new">
															<ul class="first_list1">
																
															<?php  foreach($cate['subcategory'] as $sub){ ?>	
				<li><a href="{{ URL::to('/products/'.$sub->url_key.'/'.$data->url_index.'/'.$sub->parent_id.'/'.$sub->id) }}" title="{{ $sub->category_name }}">{{ $sub->category_name }}</a></li>
															<?php } ?>
															</ul>
															</div>
															</div>
															<div class="col-md-4 left_img_info_det padding_left0">
					...
					<?php if(file_exists(base_path().'/public/assets/admin/base/images/category/'.$cate['image'])) { ?>
                        <img style="width:263px;height:125px;" alt="{{ ucfirst(strtolower($cate['category_name'])) }}"  src="<?php echo url('/assets/admin/base/images/category/'.$cate['image'].''); ?>" >
                    <?php } else{ ?>
                    ...
                    <?php } ?>
                                        ...
                                        <?php if(count($categories)>5){ ?>
											...
											<div class="col-md-4">
												...
												<div class="head_cate"> <a href="{{ URL::to('/products/'.$cate['url_key'].'/'.$data->url_index.'/'.$cate['category_id']) }}" class="category_clik" attr-id="{{ ucfirst(strtolower($cate['category_name'])) }}" title="{{ ucfirst(strtolower($cate['category_name'])) }}">{{ ucfirst(strtolower($cate['category_name'])) }}</a></div>
												<?php if(count($cate['subcategory']) > 0 ) { ?>
													<ul class="first_list1">
														<?php  foreach($cate['subcategory'] as $sub1){ ?>	
															<li><a href="{{ URL::to('/products/'.$sub1->url_key.'/'.$data->url_index.'/'.$sub1->parent_id.'/'.$sub1->id) }}" title="{{ $sub1->category_name }}">{{ $sub1->category_name }}</a></li>
														<?php } ?>
													</ul>
												<?php  }  ?>     
											</div>
                            ...
                            <div class="col-md-2 colsm-5 col-xs-5">
                                <div class="cart_sections">
                                    <a <?php if(!Session::get('user_id')){ ?> data-toggle="modal" data-target="#myModal2"  <?php } else { ?> href="{{url('cart')}}" <?php } ?> title="@lang('messages.items')"> <i class="glyph-icon flaticon-business"></i> <span class="cart_total_count_det"> <?php echo $cart_item ; ?> </span> @lang('messages.Items') </a>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="container">
                <div class="store_img">
                    ...
                    <div class="store_details_sec">
                        ...
                        <?php } ?>
                        <h2>{{ $data->outlet_name }}</h2>
                         <p>{{ $data->outlet_location_name }}</p>
                        <p>{{ $data->outlets_contact_address }}</p>
                        
                         <?php if((Session::get('current_lat') != '') && (Session::get('current_long') != '')) { ?>
                       <h4><?php $curent_lat = Session::get('current_lat');
					$current_long = Session::get('current_long');
					$outlet_lat = $data->outlets_latitude;
					$outlet_long = $data->outlets_longitude;
					$distance_cal = GetDrivingDistance($curent_lat, $current_long, $outlet_lat, $outlet_long);?>
		
				  
						<span>@lang('messages.Distance') {{ $distance_cal }} Km</span>
                   <?php } ?></h4>
                <h4>
                        <p>{{ $data->outlets_category }}</p>
                    </div>
                    <div class="store_icons">
                        <ul>
                            <li class="home info_active" ><a style="cursor:pointer;" title="@lang('messages.Home')" onclick="back_product()" ><i class="icons"><img src="{{ URL::asset('assets/front/'.Session::get('general')->theme.'/images/home_icon.png') }}" alt=""></i></a></li>
                            <li <?php if($fstatus){ ?> class="info_active" <?php } ?> ><a style="cursor:pointer;"  title="@lang('messages.Favourites')" class="favourite" <?php if(!Session::get('user_id')){ ?> data-toggle="modal" data-target="#myModal2"  <?php } else {  ?>  onclick="favourite(<?php echo $data->outlets_id;?>)" <?php } ?> ><i class="glyph-icon flaticon-favorite-1"></i></a></li>
	                       <li class="review"><a style="cursor:pointer;" title="@lang('messages.Review')" onclick="review()"><i class="glyph-icon flaticon-interface"></i></a></li>
	                       <li class="info"><a style="cursor:pointer;" title="@lang('messages.Info')" onclick="info()"><i class="glyph-icon flaticon-signs"></i></a></li>
                        </ul>
                    </div>
                </div>
                <?php if(count($store->most_sell_products)>0){ ?>
                    <div class="detail_sections">
                        <div class="bakery_sections">
                            <div class="kakery_slider">
                                <div class="bakery_title">
                                    <a><h4>@lang('messages.Most selled Products')</h4></a>
                                    <?php $sp = 0;?>
                                </div>
                                <div class="owl-carousel">
                                    <?php foreach($store->most_sell_products as $most_sell_products=>$pro){ 
                                        if($sp <=10) {  ?>
                                            <div class="item">
                                                <?php if(($pro->original_price-$pro->discount_price > 0) && ($pro->original_price > 0)) { ?>
                                                    <div class="price_dic">
                                               ...
                                               <div class="item_common_new" data-toggle="modal" data-target=".exampleModal<?php echo $pro->product_id;?>" data-whatever="@mdo">
                                                    <div class="store_list_img">
                                                        <?php if(file_exists(base_path().'/public/assets/admin/base/images/products/list/'.$pro->product_image)) { ?>
                                                            <a style="cursor:pointer; title=""> <img  title="{{ $pro->product_name }}" alt="{{ ucfirst(strtolower($pro->product_name)) }}"  <img src="<?php echo url('/assets/admin/base/images/products/list/'.$pro->product_image.''); ?>" ></a>
                                                        <?php } else{  ?>
                                                    ...
                                                    <div class="store_list_desc">
                                                        <a style="cursor:pointer; title="{{ strtolower($pro->product_name) }}">{{ ucfirst(strtolower(trim($pro->product_name))) }}  ({{ $pro->weight }}{{ $pro->unit }})</a>
                                                        <?php if($currency_side == 1) { ?>
                                                            <h3>@if(($pro->original_price-$pro->discount_price > 0) && ($pro->original_price > 0))<span class="marked_price"><?php echo  $currency_symbol; ?>{{ $pro->original_price }} </span>@endif<span class="nrl_price"><?php echo $currency_symbol; ?>{{ $pro->discount_price }} </span></h3>
                                                        <?php } else { ?>
                                                            <h3>@if(($pro->original_price-$pro->discount_price > 0) && ($pro->original_price > 0))<span class="marked_price">{{ $pro->original_price }} <?php echo $currency_symbol; ?></span>@endif<span class="nrl_price">{{ $pro->discount_price }} <?php echo $currency_symbol; ?></span></h3>
                                                        <?php } ?>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <?php if(count($data->products)>0){ ?>
                    <?php foreach($data->products as $pkey => $products) {?>
                        <div class="detail_sections">
                            <div class="bakery_sections">
                                <div class="kakery_slider">
                                    <div class="bakery_title" id="<?php echo $pkey; ?>">
                                        <a href="{{ URL::to('/products/'.$data->categories_url->$pkey->category_url_key.'/'.$data->url_index) }}"><h4><?php echo $pkey; ?></h4></a><?php $pi = 0;?>
                                    </div>
                                    <div class="owl-carousel">
                                        <?php foreach($products as $pro){ if($pi <=8) { ?>
                                            <div class="item">
                                                <?php if(($pro->original_price-$pro->discount_price > 0) && ($pro->original_price > 0)) { ?>
                                                    <div class="price_dic">
                                                        <h6>@lang('messages.Off')</h6>
                                                ...
                                                <div class="item_common_new" data-toggle="modal" data-target=".exampleModal<?php echo $pro->product_id;?>" data-whatever="@mdo">
                                                    <div class="store_list_img">
                                                        <?php if(file_exists(base_path().'/public/assets/admin/base/images/products/list/'.$pro->product_image)) { ?>
                                                            <a href="javascript:; title=""> <img  title="{{ $pro->product_name }}" alt="{{ ucfirst(strtolower($pro->product_name)) }}"  src="<?php echo url('/assets/admin/base/images/products/list/'.$pro->product_image.''); ?>" ></a>
                                                        <?php } else{  ?>
                                                            <a ref="javascript:; title=""><img src="{{ URL::asset('assets/admin/base/images/products/product.png') }}" alt="{{ ucfirst(strtolower($pro->product_name)) }}"></a>
                                                        <?php } ?>
                                                    </div>
                                                    <div class="store_list_desc">
                                                        <a ref="javascript:;  title="{{ strtolower($pro->product_name) }}">{{ ucfirst(strtolower(trim($pro->product_name))) }}  ({{ $pro->weight }}{{ $pro->unit }})</a>
                                                        <?php if($currency_side == 1) { ?>
                                                            <h3>@if(($pro->original_price-$pro->discount_price > 0) && ($pro->original_price > 0))<span class="marked_price"><?php echo  $currency_symbol; ?>{{ $pro->original_price }} </span>@endif<span class="nrl_price"><?php echo $currency_symbol; ?>{{ $pro->discount_price }} </span></h3>
                                                        <?php } else { ?>
                                                            <h3>@if(($pro->original_price-$pro->discount_price > 0) && ($pro->original_price > 0))<span class="marked_price">{{ $pro->original_price }} <?php echo $currency_symbol; ?></span>@endif<span class="nrl_price">{{ $pro->discount_price }} <?php echo $currency_symbol; ?></span></h3>
                         ...
                         <?php if($pi > 8 ) { ?>
                                <div class="view_more">
                                    <a href="{{ URL::to('/products/'.$cate['url_key'].'/'.$data->url_index.'/'.$cate['category_id']) }}" title="@lang('messages.View more')" class="btn btn-primary cancel_button">@lang('messages.View more')</a>
                                </div>
                            <?php } ?>
                        </div>
                        <div class="popup">
                            <?php foreach($products as $pro){ // print_r($pro);exit;?>
                                                ...
                                                <div class="inner_cart_sections">
                                                    ...
                                                    <div class="col-md-8">
                                                        <div class="cart_infor">
                                                            <a href="javascript:;" title="Kraft cheddar cheese spread original "><?php echo ucfirst(strtolower($pro->product_name));?> </a>
                                                            ...
                                                            <form method="POST" name="appointment_form" accept-charset="UTF-8" id="add_cart_form<?php echo $pro->product_id;?>">
                                                                ...
                                                                <div class="col-md-4 col-sm-4 col-xs-4 padding_right0">
                                                                    ...
                                                                    <?php if($currency_side == 1) { ?>
                                                                    <p class="cat_price"><?php echo $currency_symbol; ?> <span id="total_amount<?php echo $pro->product_id;?>" > {{  $pro->product_cart_count*$pro->discount_price }} </span> </p>
                    ...
                    <div class="store_info_sections">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="delevery_free">
                                ...
                                <div class="col-md-8">
                                    <h3>@lang('messages.Delivery cost')</h3>
                                    <?php if($currency_side == 1) { ?>
                            ...
                            <div class="delevery_free">
                                ...
                                <div class="col-md-8">
                                    <h3>@lang('messages.Order prepare')</h3>
                            ...
                            <div class="map_sections">
                                ...
                                <div class="social_share_info">
                                    ...
                                    <ul>
										...
										<li><a target="_blank" href="http://www.instagram.com/shareArticle?mini=true&url=<?php echo $url1; ?>" title="instagram" class="in_icon"><i class="glyph-icon flaticon-instagram-social-network-logo-of-photo-camera"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="left_deliver">
                                    ...
                                    <ul>
                                        <?php foreach($open_time as $key1 => $val1) {?>
                                            <li>
                                                <div class="col-md-6 col-sm-6 col-xs-6">
                                                    <p><?php echo trans('messages.'.$key1); ?></p>
                                                </div>
                                                <div class="col-md-6 col-sm-6 col-xs-6">
                                                    <label><?php echo (isset($val1[0]->start_time))?date("h:i a", strtotime($val1[0]->start_time)):''; ?> - <?php echo isset($val1[0]->end_time)?date("h:i a", strtotime($val1[0]->end_time)):'Leave'; ?></label>
                                                </div>
                                            </li>
                                        <?php }?>
                                    </ul>
                                ...
                                <div class="no_data_found col-md-12">
                                    <div class="no_data">
                                        <div class="">
                                            <div class="no_store_img">
                                                <img src="<?php echo URL::asset('assets/front/'.Session::get("general")->theme.'/images/no_store.png');?>" alt="">
                                                <p>@lang('messages.No reviews posted for this store.')</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                function AddtocartSubmit(product_id)
                {
                    ...
                    $.ajax({
                        ...
                        success: function (data)
                        {
                            ...
                            $(".cart_total_count_det").html(data.cart_count);
...
window.location.reload();
            ...
            $(document).ready(function() {
                ...
                $('.sqty_increase, .sqty_decrease').on('click', function() 
                {
                    ...
                    mobile = "<?php echo Session::get('mobile'); ?>";
            ...
            function favourite(vendor_id){
                ...
                $.ajax({
                    url: url,
                    headers: {'X-CSRF-TOKEN': token},
                    data: {"vendor_id":vendor_id,"user_id":user_id,"token":user_token,"language":language},
                    type: 'POST',
                    dataType:"json",
                    success: function (data){
