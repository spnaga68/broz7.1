...
@extends('layouts.app')
@section('content')
<?php //echo Session::get('current_lat');?>
        ...
        <div class="row">
                                ...
                                <form>
                                    ...
                                    @foreach ($categories as $val)
                                        <li>
                                            ...
                                            <input name="category" data-attr="{{ $val->url_key }}" id="{{ $val->id }}" class="category_ids"  value="{{ $val->id }}" <?php if(count($cate_ids)){ if(in_array($val->id,$cate_ids)) { echo  "checked"; } } else if($category_id == $val->id) {echo  "checked";} ?> type="checkbox"/> <p> {{ $val->category_name }} </p></label>
                                        </li>
                                    @endforeach
                                </form>
                            </ul>
            ...
            <div class="col-md-9 padding0">
                <div class="store_right_items">
                    ...
                    <?php if(isset($store->data) && !empty($store->data)) {  echo utf8_decode($store->data); } else { ?>
                    </span>
...
<script type="text/javascript">
    ...
    $( document ).ready(function() {
          ...
          $("#location_id").val(null).trigger("change")
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if(resp.data==''){
                        $('#location_id,#delivery_areas').append($("<option></option>").attr("value","").text('<?php echo trans('messages.No data there..');?>')); 
                    }
                    else 
                    {
      ...
      $('.category_ids').on('click', function() {
            ...
            //var ccontentType: "application/x-www-form-urlencoded;charset=utf-8";
            var category_url = "";
            ...
            var url = "<?php echo  URL::to('/store_list_ajax_front') ?>";
            $("#fadpage").show();
            $.ajax({
                url: url,
                type: "post",
                data: {"city":city,"location":location,"_token":token,"category_ids":cat_ids,"language":language,"type":"web","category_url":category_url,"user_id":user_id,"filter":1},
                dataType:"html",
                success: function(d)
                {
					$("#fadpage").hide();
                    $("#ajax_store_list").html(d);
                }
        ...
        $(".sorting").on('click', function() {
            ...
            $.ajax({
                url: url,
                type: "post",
                data: {"city":city,"location":location,"_token":token,"category_ids":cat_ids,"language":language,"keyword":keyword,"sortby":sortby,"orderby":orderby,"type":"web","filter":1},
                dataType:"html",
                success: function(d)
                {    $("#fadpage").hide();
					$("#ajax_store_list").html(d);
  ...
  var keyword = $('.sorting').val();
            var city = '<?php echo Session::get("city"); ?>';
            var location = '<?php echo Session::get("location"); ?>';
            var language = '<?php echo getCurrentLang(); ?>';
            var cat_ids = category_ids.join(",");
            var sort_id = $('.sort_active').find('.active').children().attr('id');
