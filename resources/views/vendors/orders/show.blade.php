...
<div class="media">
	...
	<div class="media-body">
		<ul class="breadcrumb">
			...
			<h4>@lang('messages.View Orders Details')</h4>
		</ul>
	</div>
</div><!-- media -->
</div><!-- pageheader -->
...
<div class="row panel panel-default">
	<div class="grid simple">
		...
		<ul class="nav nav-justified nav-wizard nav-pills">
			...
			<li @if(old('tab_info')=='delivery_info') class="active" @endif ><a href="#delivery_info" class="delivery_info" data-toggle="tab"><strong><?php  if($delivery_details[0]->order_type==1) echo trans('messages.Delivery Information'); 
									else echo trans('messages.Pickup Information'); ?></strong></a></li>
		...
		<div class="tab-content tab-content-simple mb30 no-padding" >
			<div class="tab-pane active" id="login_info">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
						  <label class="col-sm-4 control-label">{!! $vendor_info[0]->vendor_name !!}</label>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
						  ...
						  <span style="color:<?php echo $vendor_info[0]->color_code; ?>">
						  {!! $vendor_info[0]->status_name !!}</span>
						  </label>
						</div>
			...
			<div class="tab-pane" id="delivery_info">
				<legend> <?php if($delivery_details[0]->order_type==1) echo trans('messages.Delivery Information'); 
									else echo trans('messages.Pickup Information'); ?></legend>
				<div class="form-group">
					<label class="col-sm-4 control-label">@lang('messages.Name')</label>
					<div class="col-sm-8">
						<label class="col-sm-4 control-label">{!! $delivery_details[0]->user_name !!}</label>
			...
			<div class="tab-pane" id="contact_info">
				...
				<table class="table table-bordered">
					<thead>
						<tr>
						  <th class="text-left">@lang('messages.Product')</th>
						  <th class="text-right">@lang('messages.Quantity')</th>
						  <th class="text-right">@lang('messages.Unit Price')</th>
					...
					<tbody>
						...
						foreach($order_items as $items) { ?>
							<tr>
								<td class="text-left">{{$items->product_name}}</a>
			...
			<div class="tab-pane" id="history">
				...
				<fieldset>
					<legend>@lang('Add Order History')</legend>
					{!!Form::open(array('url' => ['update_delivery_status',$delivery_details[0]->order_id], 'method' => 'post','class'=>'form-horizontal','id'=>'update_delivery_status','files' => true));!!}
							...
							<div class="form-group">
							<label class="col-sm-2 control-label" for="input-order-status">@lang('Order Status')</label>
							<div class="col-sm-10">
							<select name="order_status_id" id="input-order-status" class="form-control">
								...
								<?php 
								// $change=$order_status_list['name'] = "Delivered";
							...
							<div class="form-group">
								...
								<!-- <label class="col-sm-2 control-label" for="input-notify">@lang('Notify Customer')</label>
								<div class="col-sm-10">
									<div class="checkbox"><label>
									<input name="notify" value="1" id="input-notify" type="checkbox"> -->
									<input name="order_id" value="{!! $vendor_info[0]->order_id !!}" type="hidden">
									</label></div>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label" for="input-comment">@lang('Comment')</label>
								<div class="col-sm-10">
...
$( document ).ready(function() {
	...
	$('#button-history').on('click', function() 
	{
		...
		var url = '{{url('vendors/orders/update-status')}}';
		$.ajax({
			url: url,
			...
			success: function(json) 
			{
				...
				/*if(json == 1){
					...
					$('#history-data').load('{{url('admin/orders/load_history/'.$delivery_details[0]->order_id)}}');
					toastr.error('order has been delivered')
				}*/
