...
<div class="media">
	...
	<div class="media-body">
		...
		<h4>@lang('messages.Notification Subjects')</h4>
	</div>
</div><!-- media -->
</div><!-- pageheader -->
...
<a class="btn btn-primary tip" href="{{ URL::to('admin/subjects/create') }}" title="Add New">@lang('messages.Add New')</a>
</div>
</div>
 ...
 <table id="templateTable" class="table table-striped table-bordered responsive">
    <thead>
        <tr class="headings">
            ...
            <th>@lang('messages.Subject Index')</th> 
    ...
    @foreach($subjects as $key => $value)
        <tr>
            <td>{{$i}}</td>
            <td>{{ $value->subject_index }}</td>
            <td>{{ $value->status }}</td>
			</td>
            <!-- we will also add show, edit, and delete buttons -->
            <td>
					<div class="btn-group">
                    <a href="{{ URL::to('admin/subjects/edit/' . $value->id . '') }}" class="btn btn-xs btn-white" title="@lang('messages.Edit')"><i class="fa fa-edit"></i>&nbsp;@lang('messages.Edit')</a>
...
$(function() {
    $('#templateTable').DataTable({
        ...
        columns: [
			...
			{ data: 'subject_index',name: 'subject_index'},
