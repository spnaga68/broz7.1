...
<div class="media">
	...
	<div class="media-body">
		...
		<h4>@lang('messages.Add Subject')</h4>
	</div>
</div><!-- media -->
</div><!-- pageheader -->
...
<ul class="nav nav-tabs"></ul>
	{!!Form::open(array('url' => 'createsubject', 'method' => 'post','class'=>'tab-form attribute_form','id'=>'local_edit_form','files' => true));!!}
        ...
        <div class="tab-pane active" id="home3">
                ...
                <div class="col-sm-10">
					<select class="width300 select2-offscreen" autofocus='on' id="subject_type" style="width:100%" name="subject_type">		
						...
						<?php foreach(getSubjectType() as $key => $value) { ?>
							<option value="<?php echo $key.'|'.$value; ?>" <?php if(old('subject_type')==$key.'|'.$value){  echo "selected=selected"; }  ?>><?php echo $value; ?></option>
						<?php } ?>
					</select>	                 
                </div>
            </div>	                                   
            <div class="form-group">
				...
				<select class="width300 select2-offscreen" id="template_id" style="width:100%" name="template_id">		
					...
					<?php foreach(getTemplates() as $template) { ?>						
						<option value="<?php echo $template->template_id; ?>" <?php if(old('template_id')==$template->template_id){  echo "selected=selected"; }  ?>><?php echo $template->ref_name; ?></option>
					<?php } ?>					
				</select>			                  
                </div>
            </div>
			...
			<div class="form-group">		
				...
				<div class="col-sm-10">
					<input type="text" name="color_code" class="form-control colorpicker-input" placeholder="#000000" id="colorpicker3" value="{!! old('color_code') !!}" />
