...
<div class="media">
	...
	<div class="media-body">
		...
		<h4>@lang('messages.Edit Weight Class')</h4>
	</div>
</div><!-- media -->
</div><!-- pageheader -->
...
<ul class="nav nav-tabs"></ul>
       ...
       {!!Form::open(array('url' => ['admin/update_weight_class', $data->id], 'method' => 'post','class'=>'tab-form attribute_form','id'=>'weight_class_form'));!!} 
	...
	<div class="tab-pane active" id="home3">
			...
			<div class="col-sm-10">
			  ...
			  <?php $i = 0; foreach($languages as $langid => $language):?>
                    <div class="input-group translatable_field language-<?php echo $language->id;?>" <?php if($i > 0):?>style="display: none;"<?php endif;?>>
                          <input type="text" name="weight_title[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>"  placeholder="<?php echo trans('messages.weight_title').trans('messages.'.'('.$language->name.')');?>" class="form-control" value="{{$infomodel->getLabel('title',$language->id,$data->id)}}"/>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
			  <?php $i = 0; foreach($languages as $langid => $language):?>
                    <div class="input-group translatable_field language-<?php echo $language->id;?>" <?php if($i > 0):?>style="display: none;"<?php endif;?>>
                          <input type="text" name="weight_unit[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>"  placeholder="<?php echo trans('messages.weight_unit').trans('messages.'.'('.$language->name.')');?>" class="form-control" value="{{$infomodel->getLabel('unit',$language->id,$data->id)}}"  />
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
			<input type="text"  name="weight_value" value="{!! $data->weight_value !!}"  maxlength="16" placeholder="@lang('messages.weight_value')"  class="form-control"  />
