...
<div class="pageheader">
	<div class="media">
		...
		<div class="media-body">
			...
			<h4>@lang('messages.View Vendor Details')  - {{ ucfirst($infomodel->getLabel('product_name',getAdminCurrentLang(),$data[0]->id)) }}</h4>
		</div>
	</div><!-- media -->
</div><!-- pageheader -->
  ...
  <ul class="nav nav-tabs"></ul>
	...
	<div class="buttons_block pull-right">
		<div class="btn-group mr5">
			<a class="btn btn-primary tip" href="{{ URL::to('vendor/products/edit_product/'.$data[0]->id . '') }}" title="Edit" >@lang('messages.Edit')</a>
		</div>
	</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-7">
				<?php $tcate=explode(',',$data[0]->sub_category_id); ?>
				<?php  $sub_category = getSubCategoryLists(1,$data[0]->category_id); ?>
				@foreach ($sub_category as $val)
					<?php echo in_array($val->id,$tcate)?ucfirst($val->category_name).',':''; ?>
				@endforeach
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label">@lang('messages.Product Name')</label>
			<div class="col-sm-7">{{$infomodel->getLabel('product_name',getAdminCurrentLang(),$data[0]->id)}}</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label">@lang('messages.Product URL')
			</label>
			<div class="col-sm-7">{{$infomodel->getLabel('product_url',getAdminCurrentLang(),$data[0]->id)}}</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label">@lang('messages.Description')</label>
			<div class="col-sm-7">{{$infomodel->getLabel('description',getAdminCurrentLang(),$data[0]->id)}}</div>
		</div>
		<div class="form-group">
