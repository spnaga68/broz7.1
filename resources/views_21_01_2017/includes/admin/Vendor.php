...
class Vendor extends Controller
{
    ...
    public function vendor_store(Request $data)
    {
		...
		foreach ($vendor_name  as $key => $value) {
			...
			$rules['vendor_name'.'1'] = 'required|regex:/(^[A-Za-z0-9 ]+$)+/|unique:vendors_infos,vendor_name';
		}
		...
		{ 
			...
			try{
				...
				Image::make( $destinationPath1 )->fit(200, 100)->save(base_path() .'/public/assets/admin/base/images/vendors/logos/'.$imageName)->destroy();
   ...
   /**
    ...
    public function branch_create()
    {
		if (Auth::guest())
		{
			...
			//Get countries data
			$countries = getCountryLists();
			return view('admin.outlets.create')->with('countries', $countries);
        }
    }
	/**
     * Create the specified outlet in view.
    ...
    public function branch_edit($id)
	{
		if (Auth::guest())
		{
			...
			return view('admin.outlets.edit')->with('countries', $countries)->with('data', $data)->with('infomodel', $info);
        }
    }
    /**
   ...
   /**
    ...
    public function branch_show($id)
    {
		...
		$data = DB::table('outlets')
				->select('vendors_infos.vendor_name','outlets.id','outlets.country_id','outlets.city_id','outlets.location_id','outlets.contact_address','outlets.contact_phone','outlets.contact_email','outlets.active_status','outlets.delivery_time','outlets.pickup_time','outlets.cancel_time','outlets.return_time','outlets.delivery_charges_fixed','outlets.delivery_charges_variation','outlets.service_tax','outlets.minimum_order_amount','outlet_infos.outlet_name')
    ...
    public function anyAjaxBranch()
    {
		...
		$vendors = Outlets::Leftjoin('vendors','vendors.id','=','outlets.vendor_id')
					...
					->Leftjoin('outlet_infos','outlet_infos.id','=','outlets.id')
					->select('vendors_infos.vendor_name','outlets.id','outlets.active_status','outlets.modified_date','outlets.contact_email','outlets.contact_phone','outlets.contact_address','outlets.created_date','outlets.modified_date','outlet_infos.outlet_name')
					->whereRaw($query)
					->whereRaw($query1)
