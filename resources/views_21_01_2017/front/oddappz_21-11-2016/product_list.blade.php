		...
		<div class="container">
			<div class="row">
				<div class="nav_bot_sec">
					...
					//print_r($products->categories->url_key); exit;
					...
					?>
						...
						><a href="{{ URL::to('/products/'.$cate->url_key.'/'.$outlet_info->url_index) }}" class="category_clik {{$active}}" attr-id="{{ ucfirst(strtolower($cate->category_name)) }}" title="{{ ucfirst(strtolower($cate->category_name)) }}">{{ ucfirst(strtolower($cate->category_name)) }} </a></li>
						<?php }  ?>
					<?php $i++; } ?>
					...
					<li><a href="javascript:;" title="@lang('messages.More')" id="open_drop_me">@lang('messages.More')</a>
						...
						<?php $i=1;  foreach($categories as $cate){ ?>
						<?php  if($i>6){ ?> 
								<li><a href="{{ URL::to('/products/'.$cate->url_key.'/'.$outlet_info->url_index) }}" class="category_clik" attr-id="{{ ucfirst(strtolower($cate->category_name)) }}" title="{{ ucfirst(strtolower($cate->category_name)) }}">{{ ucfirst(strtolower($cate->category_name)) }}</a></li>
						<?php  }  ?>
						<?php $i++; } ?>
						</ul>
						</div>
...
<div class="container">
	...
	<ul><li <?php if($fstatus){ ?> class="info_active" <?php } ?> ><a style="cursor:pointer;" onclick="back_product()" ><i class="icons"><img src="<?php echo URL::asset('assets/front/'.Session::get("general")->theme.'/images/home_icon.png');?>" alt=""></i></a></li>
...
if(count($products->data)){  ?>
	<?php foreach($products->data as $pro) { ?>
			...
			<div class="item_common_new" data-toggle="modal" data-target=".exampleModal<?php echo $pro->product_id;?>" data-whatever="@mdo">
				<?php if($pro->original_price-$pro->discount_price > 0) { ?>
					<div class="price_dic">
						...
						<p>{{ $pro->original_price-$pro->discount_price }} <?php echo $api->getCurrency(); ?></p>
					</div>
				<?php } ?>
		<div class="store_list_img">
				...
				<h3><span class="marked_price">{{ $pro->original_price }} <?php echo $api->getCurrency(); ?></span><span class="nrl_price">{{ $pro->discount_price }} <?php echo $api->getCurrency(); ?></span></h3>
						</div>
					</div>
		</div>
	<?php } ?>
		  ...
		  <div class="alert_info<?php echo $pro->product_id;?>"> </div>
				...
				<?php } ?>	
							...
							<div class="cart_infor"> 
									...
									<p class="cat_price"> <span id="total_amount<?php echo $pro->product_id;?>" > {{ $pro->discount_price }} </span> <?php echo $api->getCurrency(); ?></p>
									<p class="cat_price1" style="display:none;color: #e91e63;font-size: 24px;font-weight: 500;text-align: right;">  {{ $pro->discount_price }} <?php echo $api->getCurrency(); ?></p>
									</div>
...
<h4>from {{ $outlet_info->outlets_delivery_charges_fixed }} {{ getCurrency() }}</h4>
</div>
</div>
<div class="delevery_free">
