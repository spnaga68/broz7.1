					...
					<div class="col-md-9">
															...
															<div class="common_item" id="outlet_id{{$stores->outlet_id}}">
																<div class="store_itm_img">
																	<a title="" href="javascript:;">
																		<?php  if(file_exists(base_path().'/public/assets/admin/base/images/vendors/thumb/detail/'.$stores->featured_image)) { ?>
																			<img   alt="{{ ucfirst($stores->vendor_name) }}"  src="<?php echo url('/assets/admin/base/images/vendors/thumb/detail/'.$stores->featured_image.''); ?>" >
																			<?php } else{  ?>
																			<img src="<?php echo URL::asset('assets/front/'.Session::get("general")->theme.'/images/store_detial.png');?>" alt="{{ ucfirst($stores->vendor_name) }}">
																			<?php } ?>

																	</a>
																</div>
																<div class="store_itm_desc">
																	<a title="The Sultan store" href="#">{{$stores->vendor_name}}</a>
						...
						<h2><div class="rateit" data-rateit-value="{{ $stores->outlets_average_rating }}" data-rateit-ispreset="true" data-rateit-readonly="true"></div>{{ $stores->outlets_average_rating }}</h2>
														...
														<div id="no_favorites" style="{{$style_nof}}" class="col-md-4 col-sm-4 col-xs-6">
															<h3>@lang('messages.No favorites Available')</h3>
														</div>
													</div>
											</section>
										</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.favorite_store').on('click', function() 
		{
			...
			data: {"vendor_id":store_id,"user_id":user_id,"token":user_token},
			type: 'POST',
			dataType:"json",
