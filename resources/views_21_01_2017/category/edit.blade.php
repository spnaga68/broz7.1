...
<ul class="nav nav-tabs"></ul>
	...
	<div class="tab-pane active" id="home3">
			...
			<div class="form-group" id="sub_category_data" style="<?php echo ($data->category_level == 3)?'display:block;':'display:none;'; ?>">
				...
				<div class="col-sm-10">
					<select id="main_category" name="main_category" class="form-control" >
						...
						<?php $cdata = getMainCategoryLists($data->head_category_id); ?>
						@if (count($cdata) > 0)
							@foreach ($cdata as $key => $val)
								<option value="{{ $val->id }}" <?php echo ($data->main_category_id==$val->id)?'selected="selected"':''; ?>>{{ $val->category_name }}</option>
							@endforeach
						@endif
					</select>
				</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
			  <input type="number"  name="sort_order" value="{!! $data->sort_order !!}"   maxlength="10" placeholder="@lang('messages.Sort order')"  class="form-control"  />
			</div>
		</div>
		...
		<div class="form-group">
				...
				<div class="row">
					<div class="col-sm-10" style="padding-right: 0;">
                       <textarea class="form-control"  rows="3" cols="150" maxlength="250" name="description[<?php echo $language->id;?>]" id="suffix_<?php echo $language->id;?>"  placeholder="<?php echo trans('messages.Description').trans('messages.'.'('.$language->name.')');?>" class="form-control" >{{$infomodel->getLabel('description',$language->id,$data->id)}}</textarea>
					   </div>
